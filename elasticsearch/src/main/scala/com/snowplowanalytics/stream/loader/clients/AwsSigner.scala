/**
 * Copyright (c) 2014-2025 Snowplow Analytics Ltd. All rights reserved.
 *
...
case object AwsSigner {
  ...
  def apply(
    ...
    service: String,
    clock: () => LocalDateTime
  ): AwsSigner = new AwsSigner(credentialsProvider, region, service, clock)
  ...
  def getSignedHeaders(
    ...
    def sign(stringToSign: String, now: LocalDateTime, credentials: AWSCredentials): String = {
      ...
      def getSignatureKey(now: LocalDateTime, credentials: AWSCredentials): Array[Byte] = {
        val kSecret: Array[Byte] =
          (AWS4 + credentials.getAWSSecretKey).getBytes(StandardCharsets.UTF_8)
    ...
    def headerAsString(header: (String, Object), method: String): String =
      if (header._1.equalsIgnoreCase(CONNECTION)) {
        ...
        header._1.equalsIgnoreCase(CONTENT_LENGTH) && header._2.equals(ZERO) && !method
