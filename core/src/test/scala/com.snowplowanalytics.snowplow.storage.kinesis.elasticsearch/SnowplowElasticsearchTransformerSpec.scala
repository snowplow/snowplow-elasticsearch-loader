...
class SnowplowElasticsearchTransformerSpec extends Specification {
  ...
  "The 'fromClass' method" should {
    "successfully convert a tab-separated event string to JSON" in {
      ...
      // Specific fields
      result \ "platform" must_== JString("web")
      result \ "domain_sessionidx" must_== JInt(3)
      result \ "geo_latitude" must_== JDouble(37.443604)
      result \ "br_features_pdf" must_== JBool(true)
      result \ "br_features_flash" must_== JBool(false)
      result \ "collector_tstamp" must_== JString("2013-11-26T00:02:05Z")
      result \ "geo_location" must_== JString("37.443604,-122.4124")
      result \ "ti_sku" must_== JNull
