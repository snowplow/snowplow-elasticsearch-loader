...
object SnowplowTracking {
  ...
  def initializeTracker(config: SnowplowMonitoringConfig): Tracker = {
    ...
    val emitter = AsyncEmitter.createAndStart(endpoint, Some(port), config.ssl.getOrElse(false), None)
  ...
  def sendFailureEvent(
    ...
    tracker.trackSelfDescribingEvent(SelfDescribingData(
      SchemaKey("com.snowplowanalytics.monitoring.kinesis", "storage_write_failed", "jsonschema", SchemaVer.Full(1,0,0)),
      ("storage" -> StorageType) ~
  ...
  private def trackApplicationInitialization(tracker: Tracker): Unit = {
    tracker.trackSelfDescribingEvent(SelfDescribingData(
      SchemaKey("com.snowplowanalytics.monitoring.kinesis", "app_initialized", "jsonschema", SchemaVer.Full(1,0,0)),
  ...
  def trackApplicationShutdown(tracker: Tracker): Unit = {
    tracker.trackSelfDescribingEvent(SelfDescribingData(
      SchemaKey("com.snowplowanalytics.monitoring.kinesis", "app_shutdown", "jsonschema", SchemaVer.Full(1,0,0)),
      JObject(Nil)
    ))
  }
  ...
  private def trackApplicationHeartbeat(tracker: Tracker, heartbeatInterval: Long): Unit = {
    tracker.trackSelfDescribingEvent(SelfDescribingData(
      SchemaKey("com.snowplowanalytics.monitoring.kinesis", "app_heartbeat", "jsonschema", SchemaVer.Full(1,0,0)),
